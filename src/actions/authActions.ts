...
export const registerUser = (credentials: RegisterArgs) => {
    return async (dispatch: Dispatch) => {
        ...
        if ("error" in response) {
            ...
            dispatch(
                showAlert(
                    `Hi 👋 ${response.userDetails.username}. Welcome to Rohit's Whatsapp. I'm Rohit, the creator of Rohit's Whatsapp. You have me a as a friend till you invite and add your other friends 😊.`
                )
            );
        }
    };
